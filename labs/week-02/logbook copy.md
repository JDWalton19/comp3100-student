...
```
**What it means:**
that we ran the command in strace where so how many time 
## Milestone 3
**What I did:**
make -C check m3
**Output or seal:**
...
user mode - standard program run here. it what you want an the screen 
kernal mode -  progrma cannot toach anything without the sytem permmisson
that kernal act as the gatekeeper and every real action it record in the strace command  it truth even what shown on the screen.
2. **(Prompt 2 from the work order):**
because C trusts the programmer commpletely it provides  the speed and power but remove the safety like in other programming langueges. there is not nothing stoping from making fatel error c think you know what you are doing. that you have to use the agent to protct yourself from everything 
## Sources and help
...
Roughly how long this took, start to finish: 6  hours
